...
interface NotificationContextType {
  ...
  // Actions
  sendVerificationCodeNotification: (jobId: string, jobName: string, code: string) => Promise<void>;
  sendJobUpdateNotification: (jobId: string, jobName: string, update: string) => Promise<void>;
  sendApplicationStatusNotification: (jobId: string, jobName: string, status: string) => Promise<void>;
  sendMessageNotification: (conversationId: string, senderName: string, message: string) => Promise<void>;
...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({ children }) => {
    ...
    const initializeNotifications = async () => {
      try {
        ...
        if (initialized) {
          ...
          const currentPermission = await notificationService.getNotificationPermissions();
  ...
  const sendVerificationCodeNotification = async (
    ...
    try {
      await notificationService.sendVerificationCodeNotification(jobId, jobName, code);
  ...
  const sendJobUpdateNotification = async (
    ...
    try {
      await notificationService.sendJobUpdateNotification(jobId, jobName, update);
  ...
  const sendApplicationStatusNotification = async (
    ...
    try {
      await notificationService.sendApplicationStatusNotification(jobId, jobName, status);
  ...
  const sendMessageNotification = async (
    ...
    try {
      await notificationService.sendMessageNotification(conversationId, senderName, message);
