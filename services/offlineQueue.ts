...
class OfflineQueueService {
  ...
  public async enqueue(
    ...
    try {
      ...
      const existingRequest = this.queue.find(
        (req) =>
          req.idempotencyKey === idempotencyKey && req.status !== "failed"
      );
  ...
  private async processRequest(request: QueuedRequest): Promise<void> {
    try {
      ...
      console.log(
        `🔄 Processing request: ${request.endpoint} (attempt ${request.retryCount + 1}/${request.maxRetries})`
      );
  ...
  public async clearFailedRequests(): Promise<void> {
    ...
    await this.saveQueue();
    console.log(
      `🗑️ Cleared ${beforeCount - this.queue.length} failed requests`
    );
  }
  ...
  public getQueueStatus(): QueueStatus {
    return {
      ...
      pendingRequests: this.queue.filter((req) => req.status === "pending")
        .length,
      processingRequests: this.queue.filter(
        (req) => req.status === "processing"
      ).length,
      failedRequests: this.queue.filter((req) => req.status === "failed")
        .length,
