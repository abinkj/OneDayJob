...
export const useTimerRedirect = () => {
    ...
    useEffect(() => {
        ...
        if (activeJobState.job && userId && !activeJobState.loading) {
            const jobCreatorId = typeof activeJobState.job.userId === 'object' && activeJobState.job.userId !== null
            ...
            if (!isEmployer && pathname !== '/main/jobTimer') {
                console.log('🔄 [useTimerRedirect] Active job detected, redirecting to Timer screen...');
                router.replace({
