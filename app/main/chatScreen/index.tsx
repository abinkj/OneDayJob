...
export default function ChatScreen() {
  ...
  const setupSocketListeners = () => {
    ...
    socketService.on("new-message", (data: any) => {
      if (data.conversationId === conversationIdString) {
        console.log(
          "New message received - sender ID:",
          data.message.sender?.id
        );
        console.log(
          "New message received - current user ID from ref:",
      ...
      try {
        ...
        while (!socket && retryCount < maxRetries) {
          try {
            ...
            console.error(
              `Socket connection attempt ${retryCount + 1} failed:`,
              error
            );
            retryCount++;
        ...
        if (!socket) {
          console.warn(
            "Socket connection failed, continuing with REST API fallback"
          );
        }
  ...
  const sendMessage = async () => {
    ...
    console.log(
      "sendMessage called - conversationIdString:",
      conversationIdString
    );
  ...
  const handleInputChange = (text: string) => {
    ...
    if (text.length > 0 && !isTyping) {
      console.log(
        "Starting typing indicator for conversation:",
        conversationIdString
      );
      setIsTyping(true);
    ...
    typingTimeoutRef.current = setTimeout(() => {
      console.log(
        "Stopping typing indicator for conversation:",
        conversationIdString
      );
      setIsTyping(false);
      socketService.stopTyping(conversationIdString);
