      ...
      try {
        ...
        if (response.data.success) {
          ...
          if (
            ...
            if (
              sessionData.session.status === "active" &&
              sessionData?.job?.name
            ) {
              timerNotificationService.startOngoingNotification(
                sessionData.job.name
              );
            }
            ...
            dispatch(
              setActiveJob({
                ...
                employerId:
                  sessionData.employer?._id || sessionData.employer?.id,
                employerName:
                  `${sessionData.employer?.firstName || ""} ${sessionData.employer?.lastName || ""}`.trim(),
        ...
        // Handle unauthorized/pending verification (403)
          ...
          console.log(
            "[useJobSession] Pending verification or forbidden (403):",
            message
          );
  ...
  useEffect(() => {
    ...
    const handleNotification = (data: any) => {
      ...
      if (isRelevant) {
        ...
        if (
          data.type === "job_initiated" ||
          data.type === "verification_status_updated" ||
          data.type === "session_status_updated"
        ) {
          console.log(
            `[useJobSession] Relevant update received (${data.type}). Refreshing...`
          );
      ...
      if (response.data.success) {
        ...
        dispatch(
          setActiveJob({
            ...
            employerId:
              newSession.employer?._id ||
              newSession.employer?.id ||
              session?.employer?._id,
            employerName:
              `${newSession.employer?.firstName || session?.employer?.firstName || ""} ${newSession.employer?.lastName || session?.employer?.lastName || ""}`.trim(),
            employerImage:
              newSession.employer?.profilePicture ||
              session?.employer?.profilePicture,
          })
        );
  ...
  const completeSession = useCallback(
    async (notes = "") => {
      ...
      if (!session?.session?.id || session.session.status === "completed") {
        console.log(
          "[useJobSession] Session already completed or invalid, skipping completion"
        );
        return;
      }
        ...
        if (response.data.success) {
          ...
          await Promise.all([
            queryClient.invalidateQueries({
              queryKey: ["userJobs", userId, "activeCheck"],
            }),
            queryClient.invalidateQueries({
  ...
  const syncHeartbeat = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        console.warn(
          "[useJobSession] Heartbeat sync reported failure, not updating lastSyncTime"
        );
      }
    } catch (error) {
