...
export interface UseOptimisticReturn<T> {
    ...
    updateOptimistic: (optimisticData: T, serverUpdate: () => Promise<T>) => Promise<void>;
    reset: () => void;
}
 ...
 */
export function useOptimistic<T>(initialData: T | null = null): UseOptimisticReturn<T> {
    ...
    const updateOptimistic = useCallback(
        async (optimisticData: T, serverUpdate: () => Promise<T>) => {
            ...
            try {
                ...
                console.error('[useOptimistic] Server update failed, rolling back', error);
