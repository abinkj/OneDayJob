...
export type ActionType =
  "start" | "pause" | "resume" | "complete" | "heartbeat";
...
class SyncQueue {
  ...
  async processQueue(
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        if (action.retries >= this.maxRetries) {
          ...
          this.queue.push(action);
          console.log(
            `[SyncQueue] Retry ${action.retries}/${this.maxRetries} for: ${action.type}`
          );
        }
      }
    }
    ...
    console.log(
      `[SyncQueue] Processing complete. Success: ${success}, Failed: ${failed}`
    );
